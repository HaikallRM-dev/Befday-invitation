...
export default function LandingPage() {
  ...
  const whatsappNumber = '60123456789'; // Ganti dengan nombor awak
    ...
    <div style={styles.page}>
      ...
      <section style={styles.hero}>
        <div style={styles.heroContent}>
          ...
          <div style={styles.heroCTA}>
            ...
            <a href="#templates" style={styles.btnSecondary}>
              👀 View Templates
            </a>
          </div>
          <div style={styles.heroStats}>
            <div>
              ...
              <span style={{ fontSize: '0.85rem', opacity: 0.7 }}>Templates</span>
            </div>
            <div>
              <strong style={{ fontSize: '1.5rem', display: 'block' }}>RM50</strong>
              <span style={{ fontSize: '0.85rem', opacity: 0.7 }}>Starting from</span>
            </div>
            <div>
              <strong style={{ fontSize: '1.5rem', display: 'block' }}>24h</strong>
              <span style={{ fontSize: '0.85rem', opacity: 0.7 }}>Delivery</span>
            </div>
          </div>
        </div>
      </section>
            ...
            <div key={tmpl.id} style={styles.templateCard}>
              ...
              <div style={styles.templateInfo}>
                ...
                <div style={styles.templateFooter}>
                  ...
                  <a
                    href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hi! Saya nak order ${tmpl.name} (RM${tmpl.price}). Boleh bantu?`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
        ...
        <div style={styles.pricingGrid}>
          {[
            ...
            <div key={i} style={{ ...styles.pricingCard, border: pkg.popular ? '2px solid var(--primary-brown)' : '1px solid rgba(123, 94, 87, 0.15)', position: 'relative' }}>
              ...
              <a
                href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hi! Saya nak order package ${pkg.name} (RM${pkg.price}). Boleh bantu?`)}`}
                target="_blank"
                rel="noopener noreferrer"
                style={pkg.popular ? styles.btnPrimary : styles.btnSecondary}
              >
                Choose {pkg.name}
              </a>
            </div>
      ...
      <section style={styles.cta}>
        <div style={styles.ctaContent}>
          ...
          <a
            href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent('Hi! Saya berminat dengan invitation template. Boleh bantu?')}`}
            target="_blank"
            rel="noopener noreferrer"
            style={styles.btnPrimary}
          >
...
const styles = {
  page: { minHeight: '100vh', background: 'var(--bg-cream)' },

  hero: {
    background: 'linear-gradient(135deg, #F9F6F2 0%, #fdf5ed 50%, #f5e6d3 100%)',
    padding: '5rem 1.5rem',
    textAlign: 'center',
  },
  heroContent: { maxWidth: '800px', margin: '0 auto' },
  heroTag: { display: 'inline-block', background: 'rgba(123, 94, 87, 0.1)', padding: '0.4rem 1rem', borderRadius: '20px', fontSize: '0.85rem', color: 'var(--primary-brown)', marginBottom: '1.5rem' },
  heroTitle: { fontFamily: '"Playfair Display", serif', fontSize: 'clamp(2.5rem, 8vw, 4rem)', color: 'var(--primary-brown)', marginBottom: '1.5rem', lineHeight: 1.2 },
  heroSubtitle: { fontSize: '1.1rem', color: 'var(--muted-rose)', maxWidth: '600px', margin: '0 auto 2rem', lineHeight: 1.7 },
  ...
  btnPrimary: { display: 'inline-block', padding: '1rem 2.5rem', background: 'var(--primary-brown)', color: 'var(--off-white)', textDecoration: 'none', borderRadius: '50px', fontWeight: 600, fontSize: '1rem', border: 'none', cursor: 'pointer', transition: 'all 0.3s ease' },
  btnSecondary: { display: 'inline-block', padding: '1rem 2.5rem', background: 'transparent', color: 'var(--primary-brown)', textDecoration: 'none', borderRadius: '50px', fontWeight: 600, fontSize: '1rem', border: '2px solid var(--primary-brown)', cursor: 'pointer', transition: 'all 0.3s ease' },
  btnSmall: { display: 'inline-block', padding: '0.5rem 1.2rem', background: 'var(--primary-brown)', color: 'var(--off-white)', textDecoration: 'none', borderRadius: '20px', fontWeight: 600, fontSize: '0.85rem', border: 'none', cursor: 'pointer' },

  section: { padding: '4rem 1.5rem', maxWidth: '1100px', margin: '0 auto' },
  sectionTitle: { fontFamily: '"Playfair Display", serif', fontSize: 'clamp(2rem, 5vw, 2.5rem)', color: 'var(--primary-brown)', textAlign: 'center', marginBottom: '0.75rem' },
  sectionSubtitle: { fontSize: '1rem', color: 'var(--muted-rose)', textAlign: 'center', marginBottom: '3rem' },

  stepsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '2rem' },
  stepCard: { background: 'var(--card-white)', padding: '2rem', borderRadius: '20px', textAlign: 'center', boxShadow: '0 4px 16px rgba(123, 94, 87, 0.06)' },
  stepNumber: { width: '40px', height: '40px', background: 'var(--primary-brown)', color: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, margin: '0 auto 1rem' },
  stepIcon: { fontSize: '2rem', marginBottom: '0.75rem' },
  stepTitle: { fontFamily: '"Playfair Display", serif', color: 'var(--primary-brown)', marginBottom: '0.5rem' },
  stepDesc: { color: 'var(--muted-rose)', fontSize: '0.9rem', lineHeight: 1.6 },

  templateGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' },
  templateCard: { background: 'var(--card-white)', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 16px rgba(123, 94, 87, 0.06)', transition: 'all 0.3s ease' },
  templatePreview: { height: '160px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  templateEmoji: { fontSize: '4rem' },
  templateInfo: { padding: '1.5rem' },
  templateName: { fontFamily: '"Playfair Display", serif', color: 'var(--primary-brown)', marginBottom: '0.5rem' },
  templateDesc: { color: 'var(--muted-rose)', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '1rem' },
  featureList: { listStyle: 'none', padding: 0, margin: '0 0 1.5rem' },
  featureListLi: { padding: '0.3rem 0', fontSize: '0.85rem', color: 'var(--text-dark)', borderBottom: '1px solid var(--bg-cream)' },
  templateFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  price: { fontSize: '1.5rem', fontWeight: 700, color: 'var(--primary-brown)' },

  pricingGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', alignItems: 'start' },
  pricingCard: { background: 'var(--card-white)', padding: '2rem', borderRadius: '20px', textAlign: 'center' },
  popularBadge: { position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: 'var(--primary-brown)', color: 'white', padding: '0.3rem 1.2rem', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600 },
  pricingName: { fontFamily: '"Playfair Display", serif', color: 'var(--primary-brown)', marginBottom: '0.5rem' },
  pricingPrice: { fontSize: '2.5rem', fontWeight: 700, color: 'var(--primary-brown)', marginBottom: '1.5rem' },
  pricingFeatures: { listStyle: 'none', padding: 0, margin: '0 0 2rem', textAlign: 'left' },
  pricingFeaturesLi: { padding: '0.5rem 0', fontSize: '0.9rem', color: 'var(--text-dark)', borderBottom: '1px solid var(--bg-cream)' },

  faqGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' },
  faqItem: { background: 'var(--card-white)', padding: '1.5rem', borderRadius: '16px', boxShadow: '0 2px 8px rgba(123, 94, 87, 0.04)' },
  faqQ: { color: 'var(--primary-brown)', marginBottom: '0.5rem', fontSize: '0.95rem' },
  faqA: { color: 'var(--muted-rose)', fontSize: '0.9rem', lineHeight: 1.6 },

  cta: { background: 'linear-gradient(135deg, #7B5E57 0%, #302B29 100%)', padding: '4rem 1.5rem', textAlign: 'center' },
  ...
  ctaText: { fontSize: '1.1rem', color: '#D9B8A5', marginBottom: '2rem' },

  footer: { background: 'var(--dark-brown)', color: 'var(--muted-rose)', textAlign: 'center', padding: '2rem 1.5rem', fontSize: '0.85rem' },
};
